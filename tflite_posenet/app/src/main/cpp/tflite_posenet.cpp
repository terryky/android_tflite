...
int
init_tflite_posenet(ssbo_t *ssbo, const char *model_buf, size_t model_size)
{
...
decode_multiple_poses (posenet_result_t *pose_result)
{
    ...
    while (pose_result->num < MAX_POSE_NUM && !queue.empty())
    {
        ...
        keypoint_t key_points[kPoseKeyNum] = {};
...
{
    int   max_block_idx[kPoseKeyNum][2] = {};
    float max_block_cnf[kPoseKeyNum]    = {};
