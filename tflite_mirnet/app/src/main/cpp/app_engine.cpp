...
{
    int x, y, w, h;
    float *buf_fp32 = (float *)get_mirnet_input_buf (&w, &h);
...
static int
update_style_transfered_texture (mirnet_t *transfer)
{
    ...
    for (int y = 0; y < img_h; y ++)
    {
        for (int x = 0; x < img_w; x ++)
        {
            ...
            float g = *s ++;
            float b = *s ++;
            *d ++ = (uint8_t)(clamp(r) * 255);
            *d ++ = (uint8_t)(clamp(g) * 255);
            *d ++ = (uint8_t)(clamp(b) * 255);
            *d ++ = 0xFF;
        }
    }
...
AppEngine::RenderFrame ()
{
    ...
    int count = glctx.frame_count;
    {
         ...
         *  style transfer
         * --------------------------------------- */
        feed_tflite_image (&srctex, win_w, win_h);

        ttime[2] = pmeter_get_time_ms ();
        invoke_mirnet (&style_transfered);
...

        {
            ...
            draw_2d_texture_ex (&srctex, x, y, w, h, 0);
...
AppEngine::InitGLES (void)
{
    ...
    asset_read_file (m_app->activity->assetManager,
                    (char *)MIRNET_MODEL_PATH, m_tflite_model_buf);

    ret = init_tflite_mirnet (
