...
bool PinchDetector::GetPointers(float& v1x, float& v1y, float& v2x, float& v2y) {
  if (vec_pointers_.size() < 2) return false;

  int32_t index = FindIndex(event_, vec_pointers_[0]);
  if (index == -1) return false;

  v1x = AMotionEvent_getX(event_, index);
  v1y = AMotionEvent_getY(event_, index);

  index = FindIndex(event_, vec_pointers_[1]);
  if (index == -1) return false;

  v2x = AMotionEvent_getX(event_, index);
  v2y = AMotionEvent_getY(event_, index);
...
bool DragDetector::GetPointer(float& x, float& y) {
  if (vec_pointers_.size() < 1) return false;

  int32_t iIndex = FindIndex(event_, vec_pointers_[0]);
  if (iIndex == -1) return false;

  x = AMotionEvent_getX(event_, iIndex);
  y = AMotionEvent_getY(event_, iIndex);
